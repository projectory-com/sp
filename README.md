...
Compressed pipeline для мелких доработок (1-3 файла). Исследует кодовую базу, реализует фикс (opus), полирует, валидирует, записывает артефакт в fix-log. Два режима: post-flow (после task/plan/do) и standalone. Поддерживает цепочки фиксов и fix from PR comment URL. [Подробнее →](docs/fix.md)
...
Интерактивный Q&A по кодовой базе и брейнсторм. Делегирует исследование субагенту, накапливает выводы в summary chain, сохраняет exploration log. [Подробнее →](docs/explore.md)

```
/sp:explore как устроена авторизация
/sp:explore сравни подходы к кэшированию
/sp:explore что если заменить REST на gRPC
```

**Выход:** `docs/ai/<slug>/<slug>-exploration.md`

### /bootstrap — подготовка проекта к sp flow

Детектирует стек проекта, анализирует архитектуру, сканирует конвенции, генерирует CLAUDE.md и `.claude/sp-context.md`. Точка входа для подключения sp к новому проекту. [Подробнее →](docs/bootstrap.md)

```
/sp:bootstrap
/sp:bootstrap настрой sp для этого проекта
```

**Выход:** `CLAUDE.md` + `.claude/sp-context.md`
...
## Локальные скиллы (разработка)

Скиллы в `.claude/skills/` — инструменты для разработки самого плагина sp. Доступны при работе в репозитории, не входят в публикуемый плагин.

### /sp-create — фабрика скиллов

Полный pipeline создания нового скилла: анализ задачи, проектирование с mermaid-диаграммой, реализация SKILL.md и агентов, валидация качества (elements-of-style + skill-development), интеграция в документацию. [Подробнее →](docs/sp-create.md)

```
/sp-create скилл для автоматического code review с поиском багов
/sp-create https://github.com/projectory-com/sp/issues/44
```

**Выход:** `skills/<name>/SKILL.md` + агенты + docs + обновлённые README и CLAUDE.md

### /sp-release — публикация плагина

Проверки качества (проза, структура, документация, ссылки), bump версии, tag, push, GitHub release с changelog.
...
**Выход:** новый тег + GitHub release

## Telegram-нотификации

При работе с несколькими проектами параллельно (tmux + worktree) скиллы отправляют контекстные уведомления в Telegram: когда нужен ответ на вопросы, когда задача завершена, когда что-то заблокировано. [Подробнее →](docs/notify.md)

11 точек нотификации в 5 скиллах: `/task`, `/plan`, `/do`, `/fix`, `/pr`. Три типа: ACTION_REQUIRED, STAGE_COMPLETE, ALERT. Opt-in через env-переменные `CC_TELEGRAM_BOT_TOKEN` и `CC_TELEGRAM_CHAT_ID`.
...
sp/
├── .claude/
│   └── skills/              # локальные скиллы (разработка плагина)
│       ├── sp-create/       # фабрика скиллов
│       └── sp-release/      # публикация плагина
├── .claude-plugin/
...
│   ├── bootstrap/           # подготовка проекта к sp flow
│   │   ├── SKILL.md
│   │   ├── agents/          # stack-detector, architecture-mapper, convention-scanner и др.
│   │   └── reference/
│   ├── task/                # формирование задачи
│   │   ├── SKILL.md
...
│   ├── explore/             # исследование кодовой базы и брейнсторм
│   │   ├── SKILL.md
...
├── hooks/
│   ├── hooks.json           # регистрация Stop hook (Telegram notifications)
│   └── notify.sh            # скрипт доставки: читает очередь → отправляет в Telegram
├── lib/
│   └── notify.sh            # библиотека записи: скиллы вызывают для постановки в очередь
├── commands/
