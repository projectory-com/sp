...
/**
 * Context builders — translate review state into system prompts
 ...
 * - That it's operating inside the annotator (not a general coding session)
 */
 ...
 * short "you are now in review mode" preamble with the relevant content.
 */
export function buildForkPreamble(ctx: AIContext): string {
  const lines: string[] = [
    "The user is now reviewing your work and has a question.",
 ...
 * rather than a separate system-prompt channel (Claude).
 */
...
function buildPlanReviewPrompt(
  ...
  const sections: string[] = [
    "The user is reviewing an implementation plan.",
    "",
    "## Plan Under Review",
    "",
...
function buildCodeReviewPrompt(
  ...
  const sections: string[] = [
    "The user is reviewing a code diff.",
  ];
...
function buildAnnotatePrompt(
  ...
  const sections: string[] = [
    "The user is annotating a markdown document.",
    "",
