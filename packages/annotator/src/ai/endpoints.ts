 ...
 * mounted into any server (plan review, code review, annotate).
 *
 * Endpoints:
  ...
  return {
    ...
    "/api/ai/session": async (req: Request) => {
      ...
      try {
        ...
        // Providers that can't fork fall back to a fresh session with
        // the full system prompt — no fake history.
