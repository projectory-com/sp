---
name: task
description: >-
  Формирование задачи для AI-реализации. Используй когда пользователь пишет
  "создай задачу", "сделай таск", "task from ticket", "оформи задачу",
  "подготовь промт для реализации", или передаёт URL тикета / описание фичи.
---

# Формирование задачи для AI-реализации

Ты — технический prompt-инженер. Исследуй проект, пойми что нужно реализовать, сформируй промт-задачу по которому другой AI-агент выполнит реализацию с первой попытки.

Ты НЕ реализуешь задачу. Ты формулируешь её.

**WARNING:** `reference/synthesize-guide.md` занимает ~3,000 токенов. Читай его только в фазе Synthesize, непосредственно перед записью файла.

---

## Вход

`$ARGUMENTS` содержит одно или оба:
- **URL тикета** — GitHub Issues, YouTrack, Jira и др.
- **Текст задачи** — описание, примеры кода, ссылки

---

## Фазы

### Фаза 1 — Parse

**1. Получи содержимое тикета:**
- GitHub Issues → `gh issue view <url>`
- Остальные трекеры → содержимое берётся из текста пользователя, URL используется только для slug

**2. Извлеки и сохрани все материалы из входа:**
Пройдись по всему входу и выпиши отдельным списком:
- ссылки (Figma, документация, API)
- пути к файлам и скриншотам
- вставленные куски кода
- внешние ресурсы

Этот список попадёт в секцию «Материалы» без изменений.

**3. Сформируй task-slug** — идентификатор + описание на английском в kebab-case:
- `86-black-jack-page` — из GitHub issue #86
- `R2-50-user-id-to-db` — из YouTrack R2-50
- `fix-navbar-overflow` — без URL, только текст

Правило: извлеки ID из URL + 2–4 слова описания. Без URL — только описание.

**4. Определи тип задачи — frontend или нет:**

Задача **frontend** если тикет содержит хотя бы одно из:
- технологии: `React`, `Vue`, `CSS`, `Tailwind`, `SCSS`, `styled-components`, `Framer Motion`, `Three.js`
- артефакты: `component`, `page`, `layout`, `screen`, `UI`, `modal`, `drawer`, `animation`, `transition`
- визуальные: `design`, `typography`, `color`, `theme`, `dark mode`, `responsive`, `breakpoint`
- файлы: `.tsx`, `.jsx`, `.css`, `.scss` в путях из тикета

Запиши результат: `TASK_TYPE = frontend | general`

**Переход:** slug сформирован, тип определён, материалы сохранены → Фаза 2.

---

### Фаза 2 — Investigate

Агенты вызываются через Task tool. Порядок строго последовательный — architect
зависит от findings explorer.

**Шаг 1 — Предварительная оценка сложности:**
По тексту тикета определи ориентировочную сложность — это влияет на набор агентов:
- **trivial / simple** → только code-explorer
- **medium / complex** → code-explorer, затем code-architect

**Шаг 2 — Запусти code-explorer через Task tool:**

Задача агенту:
```
Исследуй кодовую базу для задачи: [вставить суть задачи из тикета].

Найди и задокументируй:
1. Все файлы и функции которые будут затронуты (пути + номера строк)
2. Паттерны и конвенции в этой части кодовой базы
3. Тесты покрывающие область изменений
4. Зависимости — что может сломаться при изменениях
5. Похожие реализации в проекте которые стоит повторить

В конце — составь список файлов ОБЯЗАТЕЛЬНЫХ для понимания темы (essential file list).
```

**Шаг 3 — Прочитай все файлы из essential file list агента.**
Не пропускать. Это строит твой контекст для Synthesize.

**Шаг 4 — Запусти code-architect через Task tool** *(только для medium / complex):*

Задача агенту:
```
На основе этих findings code-explorer: [вставить findings]

Проанализируй архитектуру области затронутой задачей: [суть задачи].

Определи:
1. Паттерны и конвенции которые нужно соблюсти
2. Точки интеграции и data flow
3. Архитектурные риски и что может сломаться
4. Если нашёл несколько несовместимых подходов — опиши оба с trade-offs

Будь конкретен: файлы, строки, имена функций.
```

**Шаг 5 — Прочитай дополнительные файлы** если code-architect расширил список.

**Критерии остановки — Investigate завершена когда:**
- [ ] Найдены точки входа (entry points) для изменений с номерами строк
- [ ] Найдены паттерны которые нужно повторить (с примерами файлов)
- [ ] Найдены или подтверждено отсутствие тестов на затронутую область
- [ ] Определено что может сломаться при изменениях

Если хотя бы один пункт не закрыт — запусти дополнительный code-explorer
с уточняющим запросом.

**Переход:** все четыре критерия закрыты → Фаза 3.

---

### Фаза 3 — Synthesize

**WARNING:** Прочитай `reference/synthesize-guide.md` прямо сейчас, перед тем как писать что-либо.

**Если `TASK_TYPE = frontend`:** дополнительно прочитай `reference/frontend-guide.md`.
Оба файла читаются один раз до начала работы, не в процессе.

Примени 5 измерений из synthesize-guide к findings фазы 2. Для каждого измерения — сначала одно предложение рассуждения вслух, потом формулировка.

**Если `TASK_TYPE = frontend`:** для измерений Requirements, Constraints и Verification дополнительно применяй frontend-контрольные списки из frontend-guide.

Определи сложность: trivial / simple / medium / complex.

Сформируй уточняющие вопросы — **ровно 5, максимум 7** — только о пробелах которые заблокируют реализацию. Не задавай вопросы ради вопросов.

**Переход:** все 5 измерений применены, вопросы готовы → Фаза 4.

---

### Фаза 4 — Write

**1.** `mkdir -p docs/ai/<task-slug>`

**2.** Запиши `docs/ai/<task-slug>/<task-slug>-task.md`:

```
# <Заголовок задачи>

**Тикет:** <URL или "—">
**Сложность:** <trivial | simple | medium | complex>

## Task

<Одно предложение — что сделать.>

## Context

<Архитектура области. Файлы с путями и номерами строк. Паттерны. Всё из фазы Investigate.>

## Requirements

1. <Конкретное, верифицируемое требование>
2. ...

## Constraints

- <Что НЕ менять>
- <Какие подходы избегать>
- <Что не ломать>

## Verification

- `<команда>` → <ожидаемый результат>
- <ожидаемое поведение>
- <edge cases>

## Уточняющие вопросы

1. **Вопрос?**
   - [ ] Вариант A — пояснение
   - [ ] Вариант B — пояснение
   - [ ] Свой вариант: ___

## Материалы

- [Описание](url)
- `path/to/file`
```

**3.** Сообщи пользователю: путь к файлу и task-slug.

---

## Правила

- Язык контента — язык оригинального тикета
- task-slug — всегда английский kebab-case
- Секция «Материалы» обязательна (если пусто — пиши "—")
- Одна задача — один файл, без декомпозиции
- Пиши активным залогом. Опускай лишние слова. Называй конкретные файлы и строки, не абстракции.
