...
Решение принимается на основе матрицы пересечений файлов из фазы Design, не на интуиции.
...
```
/sp:task → /sp:plan → /sp:do (будущий)
```

`/task` создаёт описание задачи. `/plan` строит план реализации. `/do` выполнит план.
