...
- Оркестратор с 9 субагентами — каждая фаза выполняется отдельным агентом
- Интерактивный: задаёт уточняющие вопросы через AskUserQuestion
- Точка входа для подключения sp к новому проекту

## Связи

Первый шаг перед использованием sp flow. После bootstrap доступен полный цикл: `/task` → `/plan` → `/do`.
