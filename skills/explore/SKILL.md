...
Ты — оркестратор. Принимай решения через AskUserQuestion. Файловые операции делегируй агентам.
...
- Если в предыдущих раундах есть KEY_FILES, связанные с текущим вопросом — добавь их в промт
...
- **Создать задачу через /sp:task (рекомендуется)** → вызови Skill tool с `/sp:task` и темой исследования
- **Завершить** → выйди
...
- Работай без подтверждений между фазами; AskUserQuestion только в Loop.
