...
Принимает URL тикета или текст описания. Исследует кодовую базу, анализирует архитектуру и формирует промт-задачу с контекстом, requirements, constraints и уточняющими вопросами.
...
Читает task-файл, исследует кодовую базу, принимает design decisions и декомпозирует задачу на атомарные tasks с зависимостями и порядком выполнения.
...
Делегирует tasks sub-agents, проводит двухэтапный review (spec compliance → code quality), полирует код, валидирует, обновляет документацию и пишет отчёт.
...
Анализирует изменения относительно origin/main. Формирует отчёт: ключевые участки, сложные решения, риски, вопросы к ревьюеру и сценарии ручной проверки.
