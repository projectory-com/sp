...
> Если проблемы отсутствуют — заменить таблицу текстом: **Код чист.**
...
> Если фиксы отсутствуют — заменить таблицу текстом: **Все проблемы уже исправлены.**
...
> Если пропущенные отсутствуют — заменить таблицу текстом: **Все найденные проблемы исправлены.**
...
- **Issues Found** — сортируй по Score убыванием. Severity: `high` / `medium` / `low`.
- **Fixed Issues** и **Skipped Issues** — показывай если Issues Found содержит записи.
- Пустые таблицы заменяй соответствующими заглушками из блоков `>` выше.
