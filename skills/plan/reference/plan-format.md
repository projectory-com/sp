...
Формат выходного файла `<slug>-plan.md` для Фазы 6 (Write).
...
Все tasks sequential — записать как `Task 1 → Task 2 → Task 3`.
...
- **Context в task — минимально достаточный.** Только файлы для _этого_ task.
  Конкретные пути и строки, без "прочитай весь проект" и "смотри plan-файл".
